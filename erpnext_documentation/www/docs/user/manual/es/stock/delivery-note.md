...
**Una Nota de Entrega se realiza cuando se envía un cargamento desde el Depósito de la Empresa hacia el cliente.**

Normalmente, se envía una copia de la Nota de Envio con el transportista. La Nota de 
Entrega contiene la lista de Productos que se envían en el cargamento y actualiza el inventario.
La Nota de Entrega es un paso opcional y puede crearse una Factura de Venta directamente desde una Orden de Venta. 

Para acceder al listado de Nota de Entrega ir a:
> Inicio > Existencias > Transacciones de Existencias > Nota de Entrega
...
La creación de Nota de Entrega es muy similar al Recibo de Compra. Generalmente se crea desde una Orden de Venta "Enviada" (que todavía no fue enviada al cliente) al hacer click en Crear > Envio.
...
También se puede editar la fecha y hora de publicación de la Nota de Entrega, de forma predeterminada, las mismas se establecerán en el día y la hora en que se crea la Nota de Entrega. 
...
* **Depósito y Referencia**: Se muestra el Depósito desde el cual se envían los Productos al Cliente. A su vez, si la Nota de Entrega se creó de la siguiente forma "Orden de Venta > Nota de Entrega", también se mostrará la Orden de Venta. 
...
* La Cuenta de Gastos es la cuenta desde la cuál se debitará el monto. Si se selecciona "Permitir Valoración 0", se permitirá enviar la Nota de Entrega inclusive si la Valoración del Producto es 0. Esto puede ser porque se trate de una muestra de un producto o por un acuerdo con el Proveedor.  
...
Luego de habilitar los Criterios de Inspección para Ventas en el [Formulario de Productos](/docs/user/manual/en/stock/item#216-inspection-criteria) y de adjuntar allí una Plantilla de Inspección de Calidad, las Inspecciones de Calidad pueden ser registradas en Notas de Entrega. 
...
#### Regla de envío
Una Regla de envío ayuda a determinar el costo de enviar un producto. El costo tenderá a aumentar a medida que la distancia de envío sea mayor. Para saber más, vistar la sección [Regla de envío](/docs/user/manual/en/selling/shipping-rule) page.
...
La Nota de Entrega puede relacionarse a lo siguiente a fines de contar con información más completa: 
...
#### Papel membretado
Se pueden imprimir las Notas de Entrega en papel con el membrete de la empresa. Saber más [aquí](/docs/user/manual/en/setting-up/print/letter-head).
...
de Embalaje para dividir la Nota de Entrega en unidades más pequeñas. Para saber más respecto Nota de Embalaje visitar [esta página](/docs/user/manual/en/stock/packing-slip).


Se pueden crear muchas Notas de Embalaje desde una Nota de Entrega y ERPNext se
asegurará que las cantidades en la Nota de Embalaje no excedan las cantidades
en la Nota de Entrega. Tener en cuenta que se puede crear una Nota de Embalaje desde una Nota de Entrega solo cuando la Nota de Entrega está en etapa de Borrador. 
...
* [Viaje de Entrega](/docs/user/manual/en/stock/delivery-trip)
...
> Consejo: Para deshabilitar la creación de Notas de Entrega sin Órdenes de Venta: 

### 3.18 Devolución de Orden de Venta
Una vez enviada la Orden de Venta utilizando una Nota de Entrega, se puede crear una devolución en caso de que el [Cliente](/docs/user/manual/en/CRM/customer) devuelva el Producto. Para saber más visitar la página [Devoluciones de Venta](/docs/user/manual/en/stock/sales-return).

### 3.19 Saltear Nota de Entrega

Si no se quiere crear una Nota de Entrega luego de realizada una Orden de Venta, y se desea pasar directamente a una Factura de Venta, habilitar esta opción en [Configuraciones de Venta](/docs/user/manual/en/selling/selling-settings#32-delivery-note-required).


### 4. Temas Relacionados
1. [Depósito](/docs/user/manual/en/stock/warehouse)
1. [Error de Existencias en Nota de Entrega](/docs/user/manual/en/stock/articles/delivery-note-stock-error)
1. [Transferencia de Material Desde Nota de Entrega y Recibo de Compra](/docs/user/manual/en/stock/articles/material-transfer-from-delivery-note)
