...
* **Número de Serie y Número de Lote**: Si el Producto es seriado o está en lotes, se deberá introducir el [Número de Serie](/docs/user/manual/en/stock/serial-no) y [Número de Lote](/docs/user/manual/en/stock/batch) en la tabla de Productos. Se pueden ingresar múltiples Números de Serie en un sola fila (cada uno en una línea distinta) y se deberá ingresar tantas veces el Número de Serie como la cantidad de producto a comprar/vender. 
    ...
    Se mostrarán la "Cantidad Disponible en el Almacén de Origen", la "Cantidad de Lote Disponible en el Almacén de Origen" y la "Cantidad Instalada". Para saber más respecto a instalación, visitar la página [Nota de Instalación](/docs/user/manual/en/stock/installation-note).
...
* La Cuenta de Gastos es la cuenta desde la cuál se debitará el monto. Si se selecciona "Permitir Valoración 0", se permitirá validar la Nota de Entrega inclusive si la Valoración del Producto es 0. Esto puede ser porque se trate de una muestra de un producto o por un acuerdo con el Proveedor.  
...
Para añadir impuestos de forma automática a través de una Categoría de Impuesto, visitar [esta página](/docs/user/manual/en/accounts/tax-category).
...
Si se terceriza el transporte de los Bienes hacia su lugar de entrega, los detalles del transportista pueden ser agregados. Esto no es lo mismo que [envío triangulado](/docs/user/manual/en/selling/articles/drop-shipping).
...
Si los Productos son seriados, entonces para el tipo de Productos del Paquete de Productos hay que actualizar el Número de Serie en la tabla de "Lista de Empaque". 

### 3.16 Embalaje de Productos

Si se está realizando el envío a través de distintas cajas o por peso, se puede utilizar una Nota de Embalaje para dividir la Nota de Entrega en unidades más pequeñas. Para saber más respecto Nota de Embalaje visitar [esta página](/docs/user/manual/en/stock/packing-slip).


Se pueden crear muchas Notas de Embalaje desde una Nota de Entrega y ERPNext se asegurará que las cantidades en la Nota de Embalaje no excedan las cantidades en la Nota de Entrega. Tener en cuenta que se puede crear una Nota de Embalaje desde una Nota de Entrega solo cuando la Nota de Entrega está en etapa de Borrador. 

### 3.17 Luego de Validar
...
Una vez validada la Orden de Venta utilizando una Nota de Entrega, se puede crear una devolución en caso de que el [Cliente](/docs/user/manual/en/CRM/customer) devuelva el Producto. Para saber más visitar la página [Devoluciones de Venta](/docs/user/manual/en/stock/sales-return).
